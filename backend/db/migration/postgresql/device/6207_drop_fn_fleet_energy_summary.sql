...
-- 6006_fn_energy_summary.sql) which reads tag-based rows from
-- `device_em.stats` instead of the brittle `field_group LIKE ...`
-- patterns against `device.status`. Verified zero callers in
-- `backend/src`.
--
-- The Phase 2a composite index `device__status_id_field_group_ts`
-- (migration 6205) stays — it still speeds up the env-history path
-- that Energy.Query uses.
--
