--------------UP
-- Add clearMeasurement flag to fn_virtual_meta_clear so callers can drop the
-- IEC 61850 metadata without affecting other fields.

SET search_path TO device;

DROP FUNCTION IF EXISTS device.fn_virtual_meta_clear(
    VARCHAR, VARCHAR, VARCHAR, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN
);
...
BEGIN
    UPDATE device.virtual_metadata
       SET glyph       = CASE WHEN p_clear_glyph       THEN NULL ELSE glyph       END,
           ...
           measurement = CASE WHEN p_clear_measurement THEN NULL ELSE measurement END,
