--------------UP
-- shelly_id filter now matches via the GIN-indexed shelly_ids array so
-- bulk-op rows (which leave the singular shelly_id NULL) are also found.
-- Return the shelly_ids column too so clients can show multi-device rows.
DROP FUNCTION IF EXISTS logging.fn_audit_log_query(TIMESTAMPTZ, TIMESTAMPTZ, TEXT[], VARCHAR, VARCHAR, INT, INT);
