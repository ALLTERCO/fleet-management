--------------UP
-- Per-rule delivery mode. instant = current behaviour; digest = inbox
-- recipients batched per window, overriding per-user preference.
ALTER TABLE notifications.alert_rules
    ADD COLUMN IF NOT EXISTS delivery_mode VARCHAR(16) NOT NULL DEFAULT 'instant';
ALTER TABLE notifications.alert_rules
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_schema = 'notifications'
          ...
          AND constraint_name = 'alert_rules_delivery_mode_valid'
    ) THEN
        ALTER TABLE notifications.alert_rules
            ADD CONSTRAINT alert_rules_delivery_mode_valid
    ...
    IF NOT EXISTS (
        ...
        WHERE table_schema = 'notifications'
          AND table_name = 'alert_rules'
          AND constraint_name = 'alert_rules_digest_window_positive'
    ) THEN
        ALTER TABLE notifications.alert_rules
            ADD CONSTRAINT alert_rules_digest_window_positive
            CHECK (digest_window_minutes IS NULL OR digest_window_minutes >= 1);
    END IF;
END $$;
...
BEGIN
    ...
    WITH inserted AS (
        INSERT INTO notifications.alert_rules (
            ...
            group_by, delivery_mode, digest_window_minutes
        )
        VALUES (
    ...
    SELECT
        ...
        i.delivery_mode, i.digest_window_minutes,
...
BEGIN
    ...
    SELECT
        ...
        u.delivery_mode, u.digest_window_minutes,
...
AS $$
    SELECT
        ...
        r.delivery_mode, r.digest_window_minutes,
...
AS $$
    SELECT
        ...
        r.group_by, r.delivery_mode, r.digest_window_minutes,
        COALESCE(
...
ALTER TABLE notifications.alert_rules
    DROP CONSTRAINT IF EXISTS alert_rules_delivery_mode_valid;
ALTER TABLE notifications.alert_rules
    DROP CONSTRAINT IF EXISTS alert_rules_digest_window_positive;
ALTER TABLE notifications.alert_rules
    DROP COLUMN IF EXISTS delivery_mode;
ALTER TABLE notifications.alert_rules
    DROP COLUMN IF EXISTS digest_window_minutes;
