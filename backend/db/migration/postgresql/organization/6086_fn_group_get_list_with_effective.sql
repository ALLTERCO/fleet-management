--------------UP
-- Surface effective policy values (override OR caller-supplied type default).
-- Callers pass the 4 severity-floor defaults + 4 retention-days defaults
-- (env-owned, resolved in TS). DB never reads env.
...
AS $$
    SELECT
        ...
        -- Severity floor: per-group override wins, else caller's per-type default.
        COALESCE(
            ...
            END
        ) AS effective_severity_floor,
        -- Retention days: per-group override wins, else caller's per-type default.
        COALESCE(
...
$$;
--------------DOWN
-- v2 signatures without effective_* already exist as 6083; nothing to restore.
