--------------UP
-- Extend fn_group_get / fn_group_list / fn_group_create / fn_group_update
-- to surface effective_audit_retention_days (Policy B, audit tier).
-- Shape-parity with the other effective_* columns.
...
AS $$
    SELECT
        ...
        COALESCE(
            ...
            END
        )::VARCHAR AS effective_severity_floor,
        COALESCE(
        ...
        COALESCE(
            ...
            END
        ) AS effective_audit_retention_days,
...
$$;
--------------DOWN
-- Prior signatures exist as 6089/6086; nothing to restore explicitly.
