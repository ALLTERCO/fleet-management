--------------UP
-- V4 of the location CRUD quartet. Accepts p_kind_fields JSONB and
-- p_custom_fields JSONB from the TS validator; unpacks typed columns
-- here so the SQL is the single source of truth for column mapping.
-- Replaces the flat-param fn_location_create/update and the V2/V3
-- fn_location_get/list. Old signatures dropped to avoid dispatch drift.

-- Historical signatures explicit drops (migration linter is regex-based and
-- needs to see these). The DO block below sweeps any remaining overloads in
-- case older envs have additional variants on disk.
DROP FUNCTION IF EXISTS organization.fn_location_create(
    VARCHAR, VARCHAR, VARCHAR, INTEGER, INTEGER,
    VARCHAR, JSONB, VARCHAR, JSONB, JSONB);
DROP FUNCTION IF EXISTS organization.fn_location_update(
    VARCHAR, INTEGER, VARCHAR, VARCHAR, INTEGER, BOOLEAN, INTEGER,
    VARCHAR, BOOLEAN, JSONB, VARCHAR, BOOLEAN, JSONB, JSONB);
...
-- Sweep any other overloads that drifted on disk.
DO $$
DECLARE r RECORD;
BEGIN
    FOR r IN
        ...
        WHERE n.nspname = 'organization'
          AND p.proname IN ('fn_location_create', 'fn_location_update',
                            'fn_location_get', 'fn_location_list')
    LOOP
        EXECUTE 'DROP FUNCTION ' || r.sig || ' CASCADE';
    END LOOP;
END $$;

-- Helper: unpack JSONB sub-document into column assignments.
-- Used by both create and update so field-name mapping lives in one place.
...
-- Columns used by get/list SELECT lists (kept as a single source of truth).
-- We can't declare SQL macros in Postgres — SELECT lists are inlined in
-- each fn. Keeping the order stable across create/update/get/list return
-- tables so callers can destructure by name uniformly.
...
BEGIN
    ...
    ELSIF p_parent_location_id IS NOT NULL THEN
        -- Cycle check: new parent cannot be self or a descendant.
        IF p_parent_location_id = p_id OR organization.fn_location_is_ancestor(
            p_id, p_parent_location_id
        ) THEN
...
AS $$
    ...
    SELECT
        ...
        f.environmental_setpoint, f.custom_fields,
...
--------------DOWN
-- No DOWN for v4 — prior signatures incompatible with typed columns.
-- Roll back via the 6100 typed-columns migration's DOWN.
DROP FUNCTION IF EXISTS organization.fn_location_list(
    VARCHAR, INTEGER, BOOLEAN, INTEGER, INTEGER, INTEGER[],
    BOOLEAN, VARCHAR[], INTEGER[], INTEGER[]);
DROP FUNCTION IF EXISTS organization.fn_location_get(
...
DROP FUNCTION IF EXISTS organization.fn_location_apply_kind_fields(INTEGER, JSONB);
