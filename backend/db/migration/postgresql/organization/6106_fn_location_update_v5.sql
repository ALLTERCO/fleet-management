--------------UP
-- Fix two bugs:
--   1. fn_location_is_ancestor takes 3 args, not 2 — pass p_organization_id.
--   2. RETURNS TABLE columns shadow bare WHERE refs — alias to l.*.
...
BEGIN
    ...
    ELSIF p_parent_location_id IS NOT NULL THEN
        ...
        IF p_parent_location_id = p_id OR organization.fn_location_is_ancestor(
            p_organization_id, p_id, p_parent_location_id
        ) THEN
