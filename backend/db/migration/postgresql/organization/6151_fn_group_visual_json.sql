--------------UP
-- Thread visual_json through the 4 group SQL functions. Same signatures
-- as 6094 plus one trailing column on every RETURNS TABLE, and one new
-- p_visual_json parameter on fn_group_update.
...
AS $$
    SELECT
        ...
        g.group_type, g.membership_mode, g.metadata, g.visual_json, g.is_legacy,
        COALESCE(
...
AS $$
    ...
    SELECT
        ...
        f.group_type, f.membership_mode, f.metadata, f.visual_json, f.is_legacy,
        COALESCE(
...
BEGIN
    ...
    RETURNING
        ...
        g.metadata, g.visual_json, g.is_legacy,
        (COALESCE(
            g.metadata->'policy'->>'severityFloor',
            (SELECT p.value FROM organization.group_type_policy p
              WHERE p.group_type = g.group_type AND p.field_key = 'severity_floor'),
...
DROP FUNCTION IF EXISTS organization.fn_group_update(
    VARCHAR, INTEGER, VARCHAR, VARCHAR, BOOLEAN, INTEGER, BOOLEAN, VARCHAR, JSONB, JSONB,
    VARCHAR, VARCHAR, VARCHAR, VARCHAR,
    INTEGER, INTEGER, INTEGER, INTEGER,
    INTEGER, INTEGER, INTEGER, INTEGER
);
...
$$;
--------------DOWN
-- Revert handled by re-running 6094.
