--------------UP
-- Thread image_asset_id through the 4 group SQL functions in parity
-- with visual_json (6151). New trailing column on every RETURNS TABLE
-- and one new p_image_asset_id parameter on fn_group_update.
...
BEGIN
    ...
    RETURNING
        ...
        g.metadata, g.visual_json, g.image_asset_id, g.is_legacy,
        (COALESCE(
            g.metadata->'policy'->>'severityFloor',
            (SELECT p.value FROM organization.group_type_policy p
              WHERE p.group_type = g.group_type AND p.field_key = 'severity_floor'),
...
$$;
--------------DOWN
-- Revert handled by re-running 6151.
