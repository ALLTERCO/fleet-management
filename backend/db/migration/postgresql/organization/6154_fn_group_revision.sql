--------------UP
-- Thread `revision` through the 4 group SQL functions. Update raises
-- ERRCODE='FM002' (GroupRevisionMismatch) when caller passes a stale
-- expected_revision. Get/List/Create return the current revision.
...
AS $$
    SELECT
        ...
        g.group_type, g.membership_mode, g.metadata, g.visual_json,
...
AS $$
    ...
    SELECT
        ...
        f.group_type, f.membership_mode, f.metadata, f.visual_json,
...
BEGIN
    ...
    RETURNING
        ...
        g.metadata, g.visual_json, g.image_asset_id, g.revision, g.is_legacy,
        (COALESCE(
            g.metadata->'policy'->>'severityFloor',
            (SELECT p.value FROM organization.group_type_policy p
              WHERE p.group_type = g.group_type AND p.field_key = 'severity_floor'),
...
DROP FUNCTION IF EXISTS organization.fn_group_update(
    VARCHAR, INTEGER, VARCHAR, VARCHAR, BOOLEAN, INTEGER, BOOLEAN,
...
CREATE OR REPLACE FUNCTION organization.fn_group_update(
    ...
    p_image_asset_id    VARCHAR DEFAULT NULL,
...
$$;
--------------DOWN
-- Revert handled by re-running 6152.
