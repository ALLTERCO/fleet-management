--------------UP
-- Thread `kind` through all four group SQL functions so every Group-shaped
-- row carries the column the response schema requires.
--   * fn_group_create + fn_group_update: take `p_kind` and write it inline —
--     no follow-up UPDATE, no transactional gap, no lost effective_* on
--     refetch, FK errors surface in the same statement.
--   * fn_group_get + fn_group_list: include `kind` in RETURNS TABLE so
--     Group.Get / Group.List / Group.Children responses don't return null
--     for the now-required field.
-- Input params on get/list are unchanged; only the result row grew.
...
BEGIN
    ...
    RETURNING
        ...
        g.kind, g.metadata, g.visual_json, g.image_asset_id,
        g.revision, g.is_legacy,
        (COALESCE(
...
DROP FUNCTION IF EXISTS organization.fn_group_update(
    VARCHAR, INTEGER, BIGINT, VARCHAR, VARCHAR, BOOLEAN, INTEGER, BOOLEAN,
    VARCHAR, JSONB, JSONB, VARCHAR, BOOLEAN,
    VARCHAR, VARCHAR, VARCHAR, VARCHAR,
    INTEGER, INTEGER, INTEGER, INTEGER,
    INTEGER, INTEGER, INTEGER, INTEGER
);
CREATE OR REPLACE FUNCTION organization.fn_group_update(
    ...
    p_visual_json       JSONB   DEFAULT NULL,
...
BEGIN
    ...
    SELECT g.is_legacy, g.parent_group_id, g.revision
      INTO v_is_legacy, v_current_parent, v_current_revision
      FROM organization.groups g
     WHERE g.id = p_id AND g.organization_id = p_organization_id;

    -- CAS check — fail loud if caller's expected revision is stale.
    IF p_expected_revision IS NOT NULL
       AND p_expected_revision IS DISTINCT FROM v_current_revision THEN
        RAISE EXCEPTION 'group % revision mismatch: expected %, current %',
            p_id, p_expected_revision, v_current_revision
            USING ERRCODE = 'FM002';
    END IF;
...
--------------DOWN
-- Drop all four kind-aware variants; re-running 6154's UP restores the
-- pre-kind bodies. Order matches UP for symmetry.
BEGIN;
DROP FUNCTION IF EXISTS organization.fn_group_list(
    VARCHAR, INTEGER, BOOLEAN, VARCHAR, VARCHAR, INTEGER, INTEGER,
    VARCHAR, VARCHAR, INTEGER[], BOOLEAN, VARCHAR[], INTEGER[], INTEGER[],
...
DROP FUNCTION IF EXISTS organization.fn_group_update(
    VARCHAR, INTEGER, BIGINT, VARCHAR, VARCHAR, BOOLEAN, INTEGER, BOOLEAN,
    VARCHAR, JSONB, JSONB, VARCHAR, BOOLEAN, TEXT,
    VARCHAR, VARCHAR, VARCHAR, VARCHAR,
    INTEGER, INTEGER, INTEGER, INTEGER,
    INTEGER, INTEGER, INTEGER, INTEGER
);
DROP FUNCTION IF EXISTS organization.fn_group_create(
    VARCHAR, VARCHAR, VARCHAR, INTEGER, VARCHAR, JSONB, TEXT,
    VARCHAR, VARCHAR, VARCHAR, VARCHAR,
    INTEGER, INTEGER, INTEGER, INTEGER,
    INTEGER, INTEGER, INTEGER, INTEGER
);
