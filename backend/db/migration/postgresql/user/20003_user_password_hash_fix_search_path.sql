--------------UP
-- 20002 left fn_get/fn_add/fn_update calling unqualified crypt()/gen_salt(),
-- but pgcrypto is installed in the `organization` schema — so the runtime
-- search_path doesn't see them. Re-create with schema-qualified references.
...
-- Backfill: rows hashed by 20002 used the SAME pgcrypto crypt() (the column-rename UPDATE
-- ran outside the function, so it succeeded). Existing $2a$ hashes are valid; nothing to do.

--------------DOWN
-- Re-applying 20002 would restore unqualified calls — same bug. Leave the
-- schema-qualified versions in place; rolling back only this migration is a
-- no-op (the broken state already shipped in 20002).
SELECT 1;
