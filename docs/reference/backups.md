...
| `Backup.StartDownloadJob` | Execute (devices) | `{shellyIDs, name?, contents?, idempotencyKey?}` | `{jobId}` | Queue a durable backend backup creation job. |
| `Backup.StartRestoreJob` | Execute (devices) | `{id, shellyID, restore?, idempotencyKey?}` | `{jobId}` | Queue a durable backend restore job. |
...
| `Backup.RestoreToDevice` | Execute (devices) | `{id, shellyID, restore?}` | `{success: boolean}` | Legacy direct restore RPC. UI should use `Backup.StartRestoreJob`. |
