...
Subscribes the client's socket to Fleet Manager events.
...
Returns all registered RPC components and their commands.

### Input param Validations

All methods above should have input params validations (params coming from frontend).

- Internal Ajv lib is used.
- validation rules lives in `validations.json`
- compile validations to ts: `node jsonSchema2ts.js`

example require and use:

```js
import { validator } from '../../validations';

(async () => {
    console.log(validator('FleetManagerComponent.DUMMY')({})); // this should print 'false'
})();
```
