<template>
    ...
    >
        ...
        <template #modals>
            <Modal :visible="certModalOpen" wide @close="closeCertModal">
                ...
                >
                    ...
                    <FormField label="PEM" :error="errors.pem" hint="Upload a .pem / .crt / .cer file, or paste the -----BEGIN CERTIFICATE----- block(s).">
                        <div class="cert-upload-row">
                            <label class="cert-upload-btn">
                                <i class="fas fa-upload" /> Upload file
                                <input
                                    type="file"
                    ...
                    <FormField
                        ...
                        hint="Unencrypted PKCS#8 or PKCS#1. Encrypted keys / PFX are rejected per Shelly TLS guidance."
                    >
                        <div class="cert-upload-row">
                            <label class="cert-upload-btn">
                                <i class="fas fa-upload" /> Upload key file
                                <input
                                    type="file"
                    ...
                    <template v-if="!issueResult">
                        <p class="cert-form-hint">
                            FM signs an EC P-256 leaf for the given Shelly ID against
                            the local Shelly Fleet Manager Root CA.
                        </p>
                        <FormField label="Shelly ID" :error="issueErrors.shellyId">
                            <Input
                                v-model="issueForm.shellyId"
                                placeholder="e.g. shellyplus1pm-aabbccddeeff"
                            />
                        </FormField>
                    ...
                    <template v-if="issueResult">
                        ...
                        <div class="cert-form-actions">
                            <Button type="blue-hollow" @click="downloadIssuedCert">
            ...
            <Modal :visible="detailsOpen" @close="closeDetails">
                ...
                <div v-if="detailsCert" class="cert-details">
                    ...
                    <section class="cert-details-section">
                        <button type="button" class="cert-action-btn" @click="downloadCert">
                            <i class="fas fa-download" /> Download PEM
                        </button>
                    </section>
                </div>
...
import TabSelector from '@/components/core/TabSelector.vue';
...
function _applyFilters(state: FilterState): void {
...
const certTab = ref('Issue');
const saving = ref(false);
const errors = reactive({name: '', kind: '', pem: ''});
...
const needsPrivateKey = computed(
    () =>
        importForm.kind === 'client_pair' ||
        importForm.kind === 'server_bundle' ||
        importForm.kind === 'device'
);
...
async function downloadIssuedCert(): Promise<void> {
    const cert = issueResult.value;
    if (!cert) return;
...
async function saveGroups(): Promise<void> {
    const cert = detailsCert.value;
    if (!cert) return;
    const ids = parseCsv(groupsDraft.value)
